                ...
                if (expectUsbArr) {
                    for (var i = 0; i < expectUsbArr.length; i++) {
                        ...
                        if (currentNode.node.size) {
                            var diskNodeSize = currentNode.node.size._ / 1024 / 1024 / 1024;
        ...
        function buildUSBData(usbData, tileTmpl) {
            if (usbData) {
                for (var i = 0; i < usbData.length; i++) {
                    ...
                    it.realTitle = it.title;
                    //组装usb名称和logicalname
                    if (currentNode.node.node && currentNode.node.node.node && currentNode.node.node.node.logicalname) {
                        if (typeof currentNode.node.node.node.logicalname == 'object') {
                            var usbLogicalNameIndex = currentNode.node.node.node.logicalname.length -
                                1;
                            var usbName = currentNode.node.node.node.logicalname[
                                usbLogicalNameIndex];
                    ...
                    if (currentNode.node && currentNode.node.size && currentNode.node.size._) {
