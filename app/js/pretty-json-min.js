...
PrettyJSON.tpl.Node = '' + '<span class="node-container">' +
  '<span class="node-top node-bracket" />' +
  '<span class="node-content-wrapper">' + '<ul class="node-body" />' +
  '</span>' + '<span class="node-down node-bracket" />' + '</span>';
PrettyJSON.tpl.Leaf = '' + '<span class="leaf-container">' +
...
PrettyJSON.view.Node = Backbone.View.extend({
  ...
  initialize: function(opt) {
    ...
    this.isLast = _.isUndefined(this.options.isLast) ? this.isLast : this
      .options.isLast;
  ...
  renderChilds: function() {
    ...
    _.each(this.data, function(val, key) {
      ...
      count = count + 1;
      var path = (this.type == 'array') ? this.path + '[' + key + ']' :
        this.path + '.' + key;
      var opt = {
        key: key,
        data: val,
      ...
      var child = (PrettyJSON.util.isObject(val) || _.isArray(val)) ?
        new PrettyJSON.view.Node(opt) : new PrettyJSON.view.Leaf(opt);
...
PrettyJSON.view.Leaf = Backbone.View.extend({
  ...
  mouseover: function(e) {
    e.stopPropagation();
    var path = this.path + '&nbsp;:&nbsp;<span class="' + this.type +
      '"><b>' + this.data + '</b></span>';
    this.trigger("mouseover", e, path);
  },
  mouseout: function(e) {
